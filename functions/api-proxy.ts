...
// This file exists for testing purposes - the actual deployment uses api/[[path]].ts
export const onRequest: any = async (context: any) => {
	const req = context?.request;
	const env = context?.env ?? {};
	const upstreamBase = (env?.CF_API_UPSTREAM as string) ?? 'https://ai-dnd.kopertop.workers.dev/';
