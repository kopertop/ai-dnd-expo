...
import { CactusDMProvider } from '@/services/ai/providers/cactus-dm-provider';
...
export const useCactusDungeonMaster = (options: UseCactusDungeonMasterOptions = {}) => {
	const [provider, setProvider] = useState<CactusDMProvider | null>(null);
	...
	const initialize = useCallback(async () => {
		try {
			...
			// Create provider
			const dmProvider = new CactusDMProvider({
			...
			if (success) {
				...
				console.log('✅ Cactus DM initialized successfully');
			} else {
				setError('Failed to initialize Cactus DM');
			}
		} catch (err) {
			console.error('Error initializing Cactus DM:', err);
			setError(`Failed to initialize Cactus DM: ${err instanceof Error ? err.message : String(err)}`);
		} finally {
			setIsLoading(false);
		}
	...
	const processPlayerAction = useCallback(
		async (action: string, context: DMContext): Promise<DMResponse> => {
			if (!provider || !isInitialized) {
				return {
					text: 'Cactus DM is not initialized. Please initialize first.',
					toolCommands: [],
					isLoading: false,
	...
	const generateNarration = useCallback(
		async (
			...
			if (!provider || !isInitialized) {
				return 'Cactus DM is not initialized. Please initialize first.';
			}
	...
	const cleanup = useCallback(async (): Promise<void> => {
		if (provider) {
			try {
				...
				console.error('Error cleaning up Cactus DM:', err);
			}
		}
	}, [provider]);
