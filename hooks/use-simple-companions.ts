...
const COMPANION_TEMPLATES: CompanionTemplate[] = [
	{
		...
		catchphrases: ['The trail leads this way.', 'I\'ve got your back.', 'Something doesn\'t feel right here.'],
		companionType: 'hired',
	...
	},
	{
		...
		catchphrases: ['Trust me, I\'ve got this.', 'Quietly now...', 'That was easier than expected.'],
		companionType: 'hired',
	...
	},
	{
		...
		catchphrases: ['May the light guide us.', 'I will tend to your wounds.', 'Faith will see us through.'],
		companionType: 'story',
...
export function useSimpleCompanions(): CompanionManager {
	...
	const loadAll = useCallback(async () => {
		try {
			...
			const partyConfig: PartyConfiguration = partyConfigData ? JSON.parse(partyConfigData) : {
				maxSize: 4,
				activeCompanions: [],
				leadershipStyle: 'democratic',
			};
	 ...
	 * Get companion by ID
	 */
	const getCompanion = useCallback((companionId: string): Companion | undefined => {
	 ...
	 */
	const createCompanion = useCallback(async (template: CompanionTemplate): Promise<Companion> => {
	 ...
	 */
	const addToParty = useCallback(async (companionId: string): Promise<boolean> => {
	 ...
	 */
	const removeFromParty = useCallback(async (companionId: string): Promise<boolean> => {
		const companion = getCompanion(companionId);
		...
		const newPartyConfig = {
			...state.partyConfig,
			activeCompanions: state.partyConfig.activeCompanions.filter(id => id !== companionId),
		};
	 ...
	 * Delete companion
	 */
	const deleteCompanion = useCallback(async (companionId: string): Promise<boolean> => {
	...
	const generateRandomCompanion = useCallback(async (): Promise<Companion> => {
		const template = COMPANION_TEMPLATES[Math.floor(Math.random() * COMPANION_TEMPLATES.length)];
