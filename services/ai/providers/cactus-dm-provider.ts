/**
 * Cactus DM Provider for AI D&D Platform
 *
...
export interface CactusDMProviderConfig {
	modelUrl?: string;
	contextSize?: number;
	temperature?: number;
	apiKey?: string;
...
export interface CactusDMResponse {
	text: string;
	confidence: number;
...
export class CactusDMProvider {
	private agent: CactusDMAgent;
	private config: CactusDMProviderConfig;
	private isReady = false;

	constructor(config: CactusDMProviderConfig) {
		this.config = {
			contextSize: 2048,
			temperature: 0.7,
			timeout: 15000,
		...
		this.agent = createCactusDMAgent();
	}

	/**
	 * Initialize the Cactus DM provider
	 */
	async initialize(onProgress?: (progress: number) => void): Promise<boolean> {
		try {
			...
			console.error('Failed to initialize Cactus DM provider:', error);
			this.isReady = false;
			return false;
		}
	}
	...
	/**
	 * Generate D&D response using Cactus DM agent
	 */
	async generateDnDResponse(
		...
		timeout?: number,
	): Promise<CactusDMResponse> {
		if (!this.isReady) {
			throw new Error('Cactus DM provider not initialized');
		}
		...
		try {
			...
			console.error('Error generating Cactus DM response:', error);
			throw error;
		}
	}

	/**
	 * Generate narration using Cactus DM agent
	 */
	async generateNarration(
		...
		try {
			...
			console.error('Error generating Cactus DM narration:', error);
			throw error;
		}
	}
