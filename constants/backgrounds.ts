...
export const BACKGROUND_ORIGINS: BackgroundOrigin[] = [
	{
		...
		description: 'You are a defender of earth and life, having survived the eruption of a mighty volcano. The scars you bear are a testament to your resilience and the fury of the elements.',
		races: ['Dwarf', 'Human'],
		classes: {
	...
	},
	{
		...
		description: 'You grew up wandering the endless sands, learning the ways of survival and the secrets of the desert spirits.',
	...
	},
	{
		...
		description: 'The bustling docks were your home, and the streets your school. You learned to survive by your wits and your speed.',
...
export const BACKGROUND_EVENTS: BackgroundEvent[] = [
	{
		...
		description: 'While exploring a ruin, you accidentally tripped over a switch and fell on top of an ancient artifact. The artifact became embedded next to your heart, and could not be removed. Sometimes it triggers randomly with unpredictable effects.',
		classes: ['Wizard', 'Fighter', 'Cleric'],
	},
	{
		title: 'Cursed by a Hag',
		description: 'A mysterious hag placed a powerful curse upon you. Its effects haunt your dreams and shape your destiny.',
		classes: ['Warlock', 'Cleric', 'Wizard'],
	},
	{
		title: 'Arena Survivor',
		description: 'You were forced to fight for your life in a brutal arena. The crowd cheered as you overcame impossible odds.',
...
export const BACKGROUND_GOALS: BackgroundGoal[] = [
	{
		...
		description: "As a child, your childhood friend was attacked and eaten by a large creature. You've been searching for it ever since to take out your revenge.",
		classes: ['Ranger', 'Fighter', 'Cleric'],
	},
	{
		title: 'Seek Redemption',
		description: 'You seek to atone for a past mistake or failure, hoping to restore your honor and find peace.',
		classes: ['Paladin', 'Cleric', 'Fighter'],
	},
	{
		title: 'Famous Name',
		description: 'You are determined to make your name known throughout the land, whether through heroism, cunning, or sheer force of will.',
...
export function generateRandomBackground(race: string, className: string): string {
	// Origin
	const possibleOrigins = BACKGROUND_ORIGINS.filter(o => o.races.includes(race) && Object.keys(o.classes).includes(className));
	const origin = possibleOrigins.length > 0 ? pickRandom(possibleOrigins) : pickRandom(BACKGROUND_ORIGINS);
	...
	const event = possibleEvents.length > 0 ? pickRandom(possibleEvents) : pickRandom(BACKGROUND_EVENTS);
	// Goal
	const possibleGoals = BACKGROUND_GOALS.filter(g => g.classes.includes(className));
	const goal = possibleGoals.length > 0 ? pickRandom(possibleGoals) : pickRandom(BACKGROUND_GOALS);
	return (
		`# ${origin.title}\n` +
		`${origin.description}\n${originFlavor}\n\n` +
		`# ${event.title}\n` +
