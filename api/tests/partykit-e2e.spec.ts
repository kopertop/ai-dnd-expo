...
class FakeConnection {
        ...
        constructor(public readonly request: Request, public readonly id: string = 'conn-1') {}
...
class FakeParty {
        ...
        constructor(public readonly id: string, public readonly env: CloudflareBindings) {}
        ...
        async subscribe(request: Request): Promise<Response> {
                return new Response('ok', { status: 101, headers: request.headers });
        }
}
...
describe('Partykit game room end-to-end flow', () => {
        it('authenticates, joins, moves a token, and persists via R2 SQL binding', async () => {
                ...
                const env = {
                        ...
                        QUESTS: { get: async () => null, put: async () => undefined, delete: async () => undefined, list: async () => ({ keys: [], cursor: null }) },
                ...
                const party = new FakeParty(`games/${game.invite_code}`, env) as unknown as Party<CloudflareBindings>;
                const room = new GameRoom(party, db, stateService);

                const connection = new FakeConnection(new Request('https://example.com', {
                        headers: {
                ...
                expect(connection.sent.some(msg => msg.includes('"state"'))).toBe(true);
                expect(party.messages.some(msg => msg.includes('presence'))).toBe(true);
                ...
                expect(party.messages.some(msg => msg.includes('"state"'))).toBe(true);
        });
});
